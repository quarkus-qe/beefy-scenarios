...
public class Airline extends Record{
    ...
    private static Airline from(Row row) {
        return new Airline(row.getLong(QUALIFIED_ID), row.getString(QUALIFIED_CODE_NAME), row.getString(QUALIFIED_NAME_NAME), row.getFloat(QUALIFIED_INFANT_PRICE_NAME));
    }
    ...
    public static Multi<Airline> findAll(DbPoolService client) {
        return client.query("SELECT * FROM "+client.getDatabaseName()+".airlines").execute().onItem().transformToMulti(Airline::fromSet);
    }
